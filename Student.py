...
class Student:
    def __init__(self, root):
        ...
        Left_Frame.place(x=20, y=0, width=730, height=550)
        ...
        current_course_frame.place(x=5, y=105, width=720, height=107)
...
############### function declaration ####################
        ...
        if self.var_dep.get() == "Select Department" or self.var_id.get() == "":
            ...
            try:
                conn = mysql.connector.connect(
                    host="localhost", username="root", password="5678", database="face_recognizer")
    ...
    def update_data(self):
        if self.var_dep.get() == "Select Dep" or self.var_id.get() == "" or self.var_name.get() == "":
            messagebox.showerror(
                "Error", "All Fields are required", parent=self.root)
            ...
            try:
                ...
                if update > 0:
                    ...
                    my_cursor.execute("update student set dep=%s,course=%s,year=%s,sem=%s,name=%s,roll=%s,gender=%s,dob=%s,email=%s,address=%s,phone=%s,Teacher=%s,PhotoSample=%s where id=%s" ,(
                        self.var_dep.get(),
                        self.var_course.get(),
                        self.var_year.get(),
                        self.var_sem.get(),
